...
(function() {
  ...
  function initialize() {
	...
	// shuffle track on keypress
    doc[listenFor]('keyup', function(e) {
      e.keyCode === 27 && isOpen && close(container);
      e.keyCode === 76 && isOpen && trackForward(displayVideo);
      e.keyCode === 74 && isOpen && trackBack(displayVideo);
    });
	...
	// add divs for shuffle controls
	// a bit verbose but easy to read for now

    shufback =  doc[createEl]('DIV');
    shufback.id = 'trackback';	
	var arrow = doc[createEl]('DIV');
	arrow.className = 'leftie';	
    container[appendEl](shufback);
    shufback[appendEl](arrow);
	shufback[listenFor]('click', function(e){
		if(isOpen){
			trackBack(displayVideo);
		}
		
		
	}, true); 
	shuffor =  doc[createEl]('DIV');
    shuffor.id = 'trackforward';	
	var arrow = doc[createEl]('DIV');
	arrow.className = 'rightie';
    container[appendEl](shuffor);
	shuffor[appendEl](arrow);
	shuffor[listenFor]('click', function(e){
		if(isOpen){
			trackForward(displayVideo);
		}
		
		
	}, true); 
	
	shufback.onmouseover = function(){toshuff = false;};
	shufback.onmouseout = function(){toshuff = true};
	shuffor.onmouseover = function(){toshuff = false;};
	shuffor.onmouseout = function(){toshuff = true};
	
    // all done
    initialized = true;
  }

	function click_happy(dV){
		console.log(dV);
	}
...
// hide / show loading icon
  ...
  function open(err) {
	...
	function trackForward(e){
		theNum++;
		if (theNum > links.length-1){
			theNum = 0;
		}
		displayElement.src=links[theNum];		
	}
	function trackBack(e){
		theNum--;
		if(theNum < 0){
			theNum = links.length-1;
		}
		displayElement.src=links[theNum];		
	}
  ...
  function close(e) {
	...
	// or if you are hovering over the shuffle buttons
    if (isClosing || ~clickEls.indexOf(target) || !toshuff) {
      return;
    }
